  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className={`text-2xl sm:text-3xl font-bold ${theme.content.text}`}>Customer Management</h1>
          <p className={`${theme.content.textSecondary} mt-1`}>{customers.length} customers registered</p>
        </div>
      ...
      <Card>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12">
            <p className={theme.content.textSecondary}>No customers found</p>
          </div>
        ) : (
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1">
                    <h3 className={`font-semibold ${theme.content.text} text-lg`}>{customer.name}</h3>
                    <p className={`text-sm ${theme.content.textSecondary} flex items-center mt-1`}>
                      <svg className={`w-4 h-4 mr-1 ${theme.content.textSecondary}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className={`space-y-2 text-sm ${themeName === 'dark' ? 'bg-gray-600/30' : 'bg-white/50'} rounded-lg p-3 backdrop-blur-sm`}>
                  <div className="flex justify-between items-center">
                    ...
                    <span className={`font-semibold ${theme.content.text}`}>{customer.totalPurchases}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className={`${theme.content.textSecondary} flex items-center`}>
                      <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <span className={`font-bold ${themeName === 'dark' ? 'text-emerald-400' : 'text-emerald-600'}`}>₹{customer.totalSpent.toLocaleString()}</span>
                  </div>
                  {customer.lastVisit && (
                    <div className={`flex justify-between items-center pt-2 border-t ${theme.content.cardBorder}`}>
                      ...
                      <span className={`text-xs font-medium ${theme.content.text}`}>{new Date(customer.lastVisit.toDate()).toLocaleDateString()}</span>
                    </div>
                  )}
                </div>
              </div>
